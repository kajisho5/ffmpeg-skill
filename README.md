...
**Releasing** is fully automated: bump `version` in `package.json` and add the matching `CHANGELOG.md` section in a PR, same as any other change. Once that PR merges to `main`, `.github/workflows/release.yml` takes it from there — no manual tag, no manual npm publish. It checks whether the new version already has a `vX.Y.Z` tag; if not, it creates that tag, publishes a GitHub Release with notes extracted from `CHANGELOG.md`'s matching section, and publishes the package to npm. A push to `main` that doesn't bump the version is a no-op release-wise. npm publishing needs an `NPM_TOKEN` repo secret (an npm access token with publish rights on this package) — without it the tag and GitHub Release still happen, only the npm step is skipped. A repo that depends on this one (an editing skill, an agent) should pin an `ffmpeg-skill` version by tag or npm version, not by tracking `main` — a merged-but-not-yet-released commit on `main` can be ahead of the last published npm version for the few minutes between merge and this workflow completing.
